...
static unsigned char _f_buildsectors (F_PHY * phy)
{
	...
	if (gl_volume.bootrecord.sector_per_FAT)
	{
		...
		gl_volume.root.sector = gl_volume.firstfat.sector
			+ (gl_volume.firstfat.num * (unsigned long)(gl_volume.bootrecord.number_of_FATs));
...
static unsigned char _f_prepareformat (F_PHY * phy, unsigned char fattype)
{
	...
	{
		unsigned char i;
		for (i = 0; i < (sizeof(FAT32_CS) / sizeof(t_FAT32_CS)) - 1 && phy->number_of_sectors > FAT32_CS[i].max_sectors;
				i++)
		{
		}
...
static unsigned char _f_readbootrecord (void)
{
	...
	{
		gl_volume.root.sector = gl_volume.firstfat.sector
			+ (gl_volume.firstfat.num * gl_volume.bootrecord.number_of_FATs);
		gl_volume._tdata.sector = gl_volume.root.sector + gl_volume.root.num;
		gl_volume._tdata.num = 0;
		ptr += 3;
	}
...
unsigned char fs_init (void)
{
	...
	if (span_init() != MDRIVER_SPAN_NO_ERROR)
	{
		rc = F_ERR_ONDRIVE;
	}
