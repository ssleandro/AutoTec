...
FTL_STATUS FTL_EraseAllTransLogBlocksOp (FTL_DEV devID)
{
...
#if (FTL_SUPER_SYS_EBLOCK == FTL_TRUE)
	if ((FTL_FALSE == SuperEBInfo[devID].checkLost) && (FTL_FALSE == SuperEBInfo[devID].checkSuperPF)
		&& (FTL_FALSE == SuperEBInfo[devID].checkSysPF))
	{
	...
	{
		if ((status = TABLE_GetTransLogEntry(devID, eBlockNum, &logicalAddr, &phyEBAddr, &latestIncNumber))
			!= FTL_ERR_PASS)
		{
...
//     flashPageInfo.byteCount = sizeof(sysEBlockInfo.oldSysBlock);
	...
	if ((status = TABLE_TransLogEBInsert(devID, nextLogicalEBAddr, nextPhyEBAddr, sysEBlockInfo.incNumber))
		!= FTL_ERR_PASS)
	{
		return status;
	}
...
FTL_STATUS CreateNextSuperSystemEBlockOp (FTL_DEV devID)
{
	...
	{
		if ((status = TABLE_GetSuperSysEBEntry(devID, eBlockNum, &logicalAddr, &phyEBAddr, &latestIncNumber))
			!= FTL_ERR_PASS)
		{
			return status; // trying to excess outside table.
		}
	...
	if ((status = TABLE_SuperSysEBInsert(devID, nextLogicalEBAddr, nextPhyEBAddr, sysEBlockInfo.incNumber))
		!= FTL_ERR_PASS)
	{
		return status;
	}
...
FTL_STATUS TABLE_LoadFlushTable ()
{
	...
	for (devID = 0; devID < NUM_DEVICES; devID++)
	{
		...
		for (eBlockCnt = 0; eBlockCnt < NUM_FLUSH_LOG_EBLOCKS; eBlockCnt++)
		{
...
#if(FTL_DEVICE_TYPE == FTL_DEVICE_NOR)
			for (freePageIndex = 1, validFreePageIndex = 0; freePageIndex < MAX_FLUSH_ENTRIES_PER_LOG_EBLOCK;
					freePageIndex++)
			#elif(FTL_DEVICE_TYPE == FTL_DEVICE_NAND)
...
#endif
				if ((status = GetFlushLoc(devID, phyAddr, freePageIndex, &flushStructPageInfo, &flushRAMTablePageInfo))
					!= FTL_ERR_PASS)
				{
					return status;
				}
			...
			if (0 == freePageIndex)
			{
				...
				// power failure check...
				if (FTL_TRUE
					== VerifyCheckWord((UINT16_PTR)&sysEBlockFlushInfo.type, FLUSH_INFO_DATA_WORDS,
						sysEBlockFlushInfo.checkWord))
				{
					// No more entries..
					break;
				}
	...
	if ((status = TABLE_FlushEBClear(devID)) != FTL_ERR_PASS)
	{
		...
		if (((shutdownLoc.eBlockNum == EMPTY_WORD) && (shutdownLoc.entryIndex == EMPTY_WORD))
			|| (FTL_TRUE == SuperEBInfo[devID].checkLost)) // For Power Failure Case.
		...
		if (logIndexFound != EMPTY_DWORD)
		{
			...
			{   // get rid of teh log entries, the current flush is already the latest one, dont need this 
				...
				if ((shutdownLoc.eBlockNum == EMPTY_WORD) && (shutdownLoc.entryIndex == EMPTY_WORD))
				{
					...
					if (FTL_TRUE == SuperEBInfo[devID].checkLost || FTL_TRUE == SuperEBInfo[devID].checkSuperPF
						|| FTL_TRUE == SuperEBInfo[devID].checkSysPF)
					{
						for (eBlockNum = 0; eBlockNum < NUM_SUPER_SYS_EBLOCKS; eBlockNum++)
						{
							if ((status = TABLE_GetSuperSysEBEntry(devID, eBlockNum, &logicalAddr, &phyEBAddr,
								&latestIncNumber)) != FTL_ERR_PASS)
							{
								break;   // trying to excess outside table.
							}
							if ((logicalAddr == EMPTY_WORD) && (phyEBAddr == EMPTY_WORD) && (latestIncNumber == EMPTY_DWORD))
							...
							flashPage.byteCount = OLD_SYS_BLOCK_SIGNATURE_SIZE;
							if ((flash_status = FLASH_RamPageWriteMetaData(&flashPage, (UINT8_PTR)&sysEBlockInfo.oldSysBlock))
								!= FLASH_PASS)
							{
#if(FTL_DEFECT_MANAGEMENT == FTL_TRUE)
...
FTL_STATUS FTL_FindSuperSysEB (FTL_DEV devID)
{
	...
	for (eBlockCount = SUPER_SYS_START_EBLOCKS; eBlockCount < NUM_EBLOCKS_PER_DEVICE; eBlockCount++)
	{
			...
			(sysEBlockInfo.oldSysBlock != OLD_SYS_BLOCK_SIGNATURE) && (sysEBlockInfo.phyAddrThisEBlock == phyEBlockAddr))
		{
			...
			{
				// Insert the entry, if fails check if this has higher incNum and insert that instead
				if ((status = TABLE_SuperSysEBInsert(devID, eBlockCount, phyEBlockAddr, sysEBlockInfo.incNumber))
					== FTL_ERR_FLUSH_NO_ENTRIES)
				{
					swap = FTL_FALSE;
					for (countEB = 0; countEB < NUM_SUPER_SYS_EBLOCKS; countEB++)
					{
						if ((status = TABLE_GetSuperSysEBEntry(devID, countEB, &logicalEBNumTmp, &phyEBlockAddrTmp, &key))
							!= FTL_ERR_PASS)
						{
							return status;
						}
					...
					{
						// This will clear the smallest entry
						if ((status = TABLE_SuperSysEBGetNext(devID, &logEBlockAddrOld, &phyEBlockAddrOld, NULL))
							!= FTL_ERR_PASS)
						{
							return status;
						}
...
FTL_STATUS UpdateRAMTablesUsingUnlinkLogs (FTL_DEV devID, UNLINK_LOG_ENTRY_PTR ptrUnlinkLog)
{
	...
	if (ptrUnlinkLog->partA.type == UNLINK_LOG_TYPE_A2)
	{
		for (count = 0; count < NUM_UNLINK_TYPE_B; count++)
		{
			MEM_Memcpy(&pageBitMap[count * NUM_ENTRIES_UNLINK_TYPE_B], &ptrUnlinkLog->partB[count].pageMovedBitMap[0],
				NUM_ENTRIES_UNLINK_TYPE_B);
		}
