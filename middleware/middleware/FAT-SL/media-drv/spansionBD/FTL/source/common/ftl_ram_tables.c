...
#else
				TABLE_InsertReservedEB(gcSave[index].devId,
					GetLogicalEBlockAddr(gcSave[index].devId, gcSave[index].phyEbNum));
#endif
			}
		}
...
UINT32 GetEraseCount (FTL_DEV devID, UINT16 logicalBlockNum)
{
	...
	eraseCount |= (UINT32)((EBlockMappingTable[devID][logicalBlockNum].dirtyCount & ERASE_COUNT_GET_WORD_DIRTY_MASK)
		<< ERASE_COUNT_DWORD_DIRTY_SHIFT);
	if (ERASE_STATUS_GET_WORD_MASK
...
void SetEraseCount (FTL_DEV devID, UINT16 logicalBlockNum, UINT32 eraseCount)
{
	...
	EBlockMappingTable[devID][logicalBlockNum].dirtyCount |= ((eraseCount >> ERASE_COUNT_DWORD_DIRTY_SHIFT)
		& ERASE_COUNT_GET_WORD_DIRTY_MASK);
#endif
	MarkEBlockMappingTableEntryDirty(devID, logicalBlockNum);
}
...
#else
		bitValue = (FREE_BIT_MAP_TYPE)GetBitMapField(
			(UINT8_PTR)(&(EBlockMappingTable[devID][logicalEBNum].freeBitMap[0])),
			temp, FTL_BITS_PER_PAGE);
#endif
...
#else
			bitValue = (FREE_BIT_MAP_TYPE)GetBitMapField(
				(UINT8_PTR)(&(EBlockMappingTable[devID][logicalBlockNum].freeBitMap[0])),
				temp, FTL_BITS_PER_PAGE);
#endif
			if (bitValue == BLOCK_INFO_EMPTY_PAGE)
			{
...
#endif // #if(FTL_EBLOCK_CHAINING == FTL_TRUE)
#else    
	SetBitMapField(&EBlockMappingTable[devID][logicalBlockNum].freeBitMap[0], oldPageIdx, FTL_BITS_PER_PAGE,
		BLOCK_INFO_STALE_PAGE);
...
void TABLE_ClearMappingTable (FTL_DEV devID, UINT16 logicalEBNum, UINT16 phyEBAddr, UINT32 eraseCount)
{
	...
	EBlockMappingTable[devID][logicalEBNum].dirtyCount |= ((eraseCount >> ERASE_COUNT_DWORD_DIRTY_SHIFT)
		& ERASE_COUNT_GET_WORD_DIRTY_MASK);
#endif
...
FTL_STATUS FindAndSwapUnusedEB (FTL_DEV devID, UINT16 logFrom, UINT16_PTR logTo)
{
	...
	TRANSFER_EB transferEB =
	{	EMPTY_BYTE, EMPTY_WORD}; /*3*/
	FTL_STATUS status = FTL_ERR_PASS; /*4*/
