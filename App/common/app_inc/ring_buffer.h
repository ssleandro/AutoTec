...
/**
 * @def   RB_PEEK_LAST(RingBuff)
 * @param RingBuff RingBuffer
 * Peek last inserted item without removing it from the list
 */
#define RB_PEEK_LAST(type, RingBuff)       ((type*)RingBuff.data + ((RingBuff.head - 1) & (RingBuff.count - 1)))
//#define RB_PEEK_LAST(type, RingBuff)       ((type*)RingBuff.data + (((RingBuff.head - 1) & (RingBuff.count - 1)) * RingBuff.itemSz))
/**
 * @def		RB_VHEAD(rb)
