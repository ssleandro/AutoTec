...
#define LPC_USB_BASE(x)            (sUSBMap[x].wLPCUSBBase)//LPC_USB0_BASE
#define LPC_USB(x)                 (sUSBMap[x].pUSB)//LPC_USB0
#define LPC_USB_IRQ(x)             (sUSBMap[x].eLPCUSBIRQ)//USB0_IRQn
...
static ErrorCode_t VCOM_bulk_in_hdlr (USBD_HANDLE_T hUsb, void *data, uint32_t event)
{
	...
	if ((event == USB_EVT_RESET) || (event == USB_EVT_DEV_ERROR))
	{
		/* Calls the API function to disable USB */
		USBD_API->hw->ResetEP(hUsb, USB_CDC_IN_EP);
	}
	return LPC_OK;
}
...
static ErrorCode_t VCOM_bulk_out_hdlr (USBD_HANDLE_T hUsb, void *data, uint32_t event)
{
	...
	switch (event)
	{
		...
		/* Calls the API function to disable USB */
		USBD_API->hw->ResetEP(hUsb, USB_CDC_OUT_EP);
			break;
		default:
			break;
	}
...
uint32_t vcom_bread (eUSBInterfaces_s eUSBInterface, uint8_t *pBuf, uint32_t buf_len)
{
	VCOM_DATA_T *pVcom = &g_vCOM;
	uint16_t cnt = 0;
	...
	if (pVcom->rx_count)
	{
		...
		NVIC_DisableIRQ(LPC_USB_IRQ(eUSBInterface));
		...
		NVIC_EnableIRQ(LPC_USB_IRQ(eUSBInterface));
	}
	return cnt;
}

/* Virtual com port read routine */
ErrorCode_t vcom_read_req (eUSBInterfaces_s eUSBInterface, uint8_t *pBuf, uint32_t len)
{
	VCOM_DATA_T *pVcom = &g_vCOM;
...
uint32_t vcom_write (eUSBInterfaces_s eUSBInterface, uint8_t *pBuf, uint32_t len)
{
	VCOM_DATA_T *pVcom = &g_vCOM;
	uint32_t ret = 0;
