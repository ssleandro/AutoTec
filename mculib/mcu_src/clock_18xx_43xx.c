	...
	for (n = 1; n <= 4; n++)
	{
		for (p = 0; p < 4; p++)
		{
			for (m = 1; m <= 256; m++)
			{
				...
				if (fcco < PLL_MIN_CCO_FREQ)
					continue;
				if (fcco > PLL_MAX_CCO_FREQ)
					break;
				if (ppll->ctrl & (1 << 7))
				{
					fout = fcco;
...
uint32_t Chip_Clock_SetupMainPLLHz (CHIP_CGU_CLKIN_T Input, uint32_t MinHz, uint32_t DesiredHz, uint32_t MaxHz)
{
	...
	if (DesiredHz != 0xFFFFFFFF)
	{
		...
		if (freqout && !freqout2)
		{ /* rounding up is no good? set first multiplier */
			Mult--;
			return Chip_Clock_SetupMainPLLMult(Input, Mult);
		}
		if (!freqout && freqout2)
		{ /* didn't work until rounded up? set 2nd multiplier */
			return Chip_Clock_SetupMainPLLMult(Input, Mult);
		}

		if (freqout && freqout2)
		{ /* either multiplier okay? choose closer one */
